...
def _total_ram_bytes():
    ...
    Returns 8GB so the cap is still applied even if detection fails.
    """
    try:
...
def _default_mem_limit():
    """Cap each child at 30% of total RAM, leaving headroom for OS and the parent process."""
...
# Defaults to 30% of total RAM. Override by setting CHILD_MEM_LIMIT_GB.
...
def run_with_timeout(worker_fn, args, timeout):
    """
    Run worker_fn() in a child process, hard-killing it after timeout
    ...
    added by the worker on success.
    """
    ...
    # Record wall time before kill overhead.
...
def load_completed(csv_path):
    """
    Return a set of (scale, instance, run_type) tuples already recorded in
